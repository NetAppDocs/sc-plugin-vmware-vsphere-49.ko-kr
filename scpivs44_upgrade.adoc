...
업그레이드 프로세스에서는 기존 플러그인의 등록을 해제하고 vSphere 7.0U1 이상 버전에서만 호환되는 플러그인을 구축합니다.
...
+
.. NetApp Support 사이트에 로그인합니다 (https://mysupport.netapp.com/products/index.html[])를 클릭합니다.
...
.. 업로드 팝업 화면에서 을 다운로드한 위치로 이동합니다 `.iso` 파일을 클릭한 다음 를 클릭합니다 `.iso` 파일 이미지를 클릭한 다음 * 열기 * 를 클릭합니다.
파일이 데이터 저장소에 업로드됩니다.
.. 업그레이드하려는 VM으로 다시 이동한 다음 * 요약 * 탭을 클릭합니다.
...
... 목차 열에서 로 이동합니다 `.iso` 업로드한 파일을 파일 형식 필드에서 "ISO 이미지"가 선택되어 있는지 확인한 다음 * 확인 * 을 클릭합니다.
