...
* `uri` 매개 변수
+
를 클릭합니다 `uri` 매개 변수는 항상 "null" 값을 반환합니다.
...
+
토큰을 얻으려면 를 사용합니다 `/4.1/auth/login` REST API를 사용하여 vCenter 자격 증명을 제공합니다.

* API 버전 지정
+
각 REST API 이름은 REST API가 처음 릴리스된 SnapCenter 버전 번호를 포함합니다. 예를 들어 REST API를 입력합니다 `/4.1/datastores/{moref}/backups` SnapCenter 4.1에서 처음 출시되었습니다.
